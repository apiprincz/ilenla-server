...
router.get("/merchantReceipts/:id", cleanBody, async (req, res) => {
  try {
    ...
    const statusArr = await MerchantReceipt.find(
      { merchant: id },
      { _id: 0, active: true, _id: true }
    );
...
router.post("/receipts", cleanBody, async (req, res) => {
  try {
    ...
    merchantReceipt.code = `WD-${
      merchantReceipt.discount
    }%OFF-${merchantReceipt.title.slice(0, 6)}-${code}`;

    let today = new Date();
    if (
      today.getTime() >= new Date(merchantReceipt.startDate).getTime() &&
      today.getTime() <= new Date(merchantReceipt.endDate).getTime()
    ) {
      merchantReceipt.active = true;
    }

    const newMerchantReceipt = await merchantReceipt.save();
    return res.status(200).json({
      success: true,
      message: "Receipt Campaign Created!! add image",
...
router.patch("/receipts/:id", async (req, res) => {
  ...
  const updatedReceipt = await MerchantReceipt.findByIdAndUpdate(_id, receipt, {
    new: true,
  });
  ...
  return res.status(200).json({
    ...
    merchantReceipts: updatedReceipt,
  });
});
